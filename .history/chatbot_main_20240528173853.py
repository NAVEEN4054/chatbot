...
def solve_math_query(query):
    try:
        # Clean up the query to make it parseable by sympy
        query = query.replace('^', '**')
        query = re.sub(r'[^0-9\+\-\*/\=\(\)x\s\*\*]', '', query)
        if '=' in query:
            lhs, rhs = query.split('=')
            equation = sp.Eq(sp.sympify(lhs), sp.sympify(rhs))
            solution = sp.solve(equation)
        else:
            equation = sp.sympify(query)
            solution = sp.solve(equation)
...
def response(user_response):
    if is_math_query(user_response):
        return solve_math_query(user_response)
    robo_response = ''
